...
export default function App() {
  ...
  const [selectedDetailProduct, setSelectedDetailProduct] = useState(() => {
    const saved = localStorage.getItem('selectedDetailProduct');
    return saved ? JSON.parse(saved) : null;
  });
  ...
  useEffect(() => {
    ...
    if (currentView === 'detail' && !selectedDetailProduct) {
      setCurrentView('home');
    }
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 sm:space-y-10">
        ...
        {currentView === 'dashboard' ? (
          <Dashboard />
        ) : currentView === 'detail' && selectedDetailProduct ? (
          <ProductDetailPage
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredProducts.map((product) => (
                    <ProductCard
                      key={product.id}
                      product={product}
                      onSelectForConsult={handleOpenProductDetail}
                    />
                  ))}
                </div>
              )}
